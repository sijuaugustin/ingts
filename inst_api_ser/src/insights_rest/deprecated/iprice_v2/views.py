...
class iPriceTrendViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        """
        iPrice Trend Version 2.0 with  property types

           1.'TownHouse',
           2.'Multifamily'
           3.'All'

        and response data is in weekly format.

        url:https://insights.propmix.io:8004/iprice/v2/trend
        ---
        ...
        data = list(db_client.iestimate.mls_data_new_nxmodel2_w_trend.find({"postalcode": str(request.query_params['zip'])}))
        if len(data) == 0:
...
class iZipStatusViewSet(viewsets.ViewSet):
    def list(self, request):
        """
        iPrice Trend Version 2.0 zipStatus
        ...
        """
        db_client = MongoClient('mongo-master.propmix.io', 33017)
        zip_status = db_client.iestimate.mls_data_new_zipstatus.find_one({"version": "v2"})['ministatus']
